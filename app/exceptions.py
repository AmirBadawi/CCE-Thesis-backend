...
async def generate_error_response(ex):
    ...
    try:
        ...
        response = await asyncio.wait_for(
            llm.ainvoke(
                f"As KU AI Virtual Assistant, generate a brief user-friendly error message for the following:\n{ex}"
            ),
            timeout=10,
        )
        response = response.content
...
async def generate_userfriendly_error_response():
    ...
    try:
        ...
        response = await asyncio.wait_for(
            llm.ainvoke(
                f"As KU AI Virtual Assistant, generate a brief user friendly error response to try again or contact support."
            ),
            timeout=10,
        )
        response = response.content
